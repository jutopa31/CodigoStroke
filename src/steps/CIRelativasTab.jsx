      ...
      <div className="flex items-center gap-2 px-3 py-1.5">
        ...
        <div className="flex shrink-0 rounded-md overflow-hidden border border-stroke-line text-[11px] font-bold">
          ...
          <button type="button" onClick={() => onChange(true)}
            className={`px-2.5 py-1 transition active:scale-95 ${isYes ? 'bg-amber-500 text-stroke-bg' : 'bg-stroke-navy text-stroke-textMuted hover:bg-stroke-bg'}`}>
            SÍ
          </button>
        </div>
      </div>
    ...
    <div className="px-4 pb-6 space-y-2.5 md:px-0">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5">
          ...
          <div className="flex shrink-0 rounded-md overflow-hidden border border-stroke-line text-[11px] font-bold">
            <button type="button" onClick={() => setAnticoagActive(false)}
              className={`px-2.5 py-1 transition active:scale-95 ${anticoag.active === false ? 'bg-slate-600 text-white' : 'bg-stroke-navy text-stroke-textMuted hover:bg-stroke-bg'}`}>
              NO
            </button>
            <div className="w-px bg-stroke-panel" />
            <button type="button" onClick={() => setAnticoagActive(true)}
              className={`px-2.5 py-1 transition active:scale-95 ${anticoag.active === true ? 'bg-amber-500 text-stroke-bg' : 'bg-stroke-navy text-stroke-textMuted hover:bg-stroke-bg'}`}>
              SÍ
            </button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {ANTICOAG_TYPES.map(({ id, label }) => (
                <button key={id} type="button" onClick={() => setAnticoagType(id)}
                  className={`py-2 rounded-lg border text-xs font-semibold transition active:scale-95 ${
                    anticoag.type === id
      ...
      <div className="flex items-center gap-2 pt-1">
        <div className="flex-1 bg-stroke-panel rounded-full h-1.5 overflow-hidden">
          <div className="bg-amber-400 h-1.5 rounded-full transition duration-300"
