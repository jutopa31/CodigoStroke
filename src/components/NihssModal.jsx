...
export default function NihssModal({ onLoad, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/60 animate-fade-in">
      <div className="bg-stroke-navy w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-slide-down">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h2 className="font-display text-stroke-text text-lg">Calculadora NIHSS</h2>
            <p className="text-xs text-stroke-textMuted mt-0.5">
              {answered}/{nihssItems.length} ítems completados
            </p>
          </div>
          <button onClick={onClose} className="text-stroke-textMuted hover:text-stroke-textMuted">
            <X size={20} />
          </button>
        </div>
        ...
        {allDone && (
          <div className={`mx-5 mb-3 rounded-xl border px-4 py-3 ${severity.bg} ${severity.border} animate-fade-in`}>
            <p className="text-xs text-stroke-textMuted mb-1">Puntaje total</p>
        ...
        <div className="px-5 pb-5 flex gap-3">
          <button
            ...
            disabled={current === 0}
            className="px-4 py-3 border border-stroke-line rounded-xl text-stroke-textMuted hover:bg-stroke-bg disabled:opacity-40 transition-colors"
          >
            <ChevronLeft size={18} />
          </button>
          ...
          {current < nihssItems.length - 1 ? (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1 py-3 border border-stroke-line rounded-xl text-stroke-textMuted text-sm hover:bg-stroke-bg transition-colors"
            >
